...
class AsymmetricLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # log_value = torch.log(xs_neg.clamp(min=self.eps)).detach().cpu().numpy()
        # xs_neg_value = xs_neg.detach().cpu().numpy()
        # print(log_value)
        # print(xs_neg_value)
        # import numpy as np
        # idx = np.where(log_value == float("-inf"), xs_neg_value, log_value)
        # print(idx)
        # print(xs_neg.clamp(min=self.eps, max=1))
        # Basic CE calculation
        los_pos = y * torch.log(xs_pos.clamp(min=self.eps))
        los_neg = (1 - y) * torch.log(xs_neg.clamp(min=self.eps))
        loss = (los_pos + los_neg) / (xs_pos.size()[0] * xs_pos.size()[1])
        # Asymmetric Focusing
...
class AsymmetricLossOptimized(nn.Module):
    ...
    def forward(self, x, y):
        ...
        self.loss = self.loss / (self.xs_pos.size()[0] * self.xs_pos.size()[1])
