...
class Runner(object):
    ...
    def train_epoch(self, epoch, model, loader, optimizer, loss_fns, args, model_ema=None,
                    ...
                    loss_scaler=None, writer_dict=None, schd_batch_update=False):
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            for i, out in enumerate(output):
                ...
                if mixup_fn is not None or args.soft_ce_loss:
        ...
        if lr_scheduler is not None and not schd_batch_update:
            lr_scheduler.step()
    ...
    def validate(self, model, loader, loss_fn, args, log_suffix='', output_dir='', amp_autocast=suppress,
                 writer_dict=None, show_feats=[], verbose=False, ignore=False):
    ...
    def compute_metrics(self, args, names, show_feats=[], verbose=False, return_weights=False, ignore=False):
        weights = {}
        ...
        for i in range(args.head_num):
            ...
            if return_weights:
                ...
                weights[self.classes_list[i]] = {}
                weights[self.classes_list[i]]["model_weight"] = tf1
                weights[self.classes_list[i]]["label_weight"] = f1_list
        ...
        if return_weights:
            return np.mean(mean_style_f1), np.mean(mean_color_f1), np.mean(mean_f1), np.mean(mean_ig_f1), weights
        else:
