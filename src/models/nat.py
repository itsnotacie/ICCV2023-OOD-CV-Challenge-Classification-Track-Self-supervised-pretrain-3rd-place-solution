...
from src.models.cuda.natten import NeighborhoodAttention
...
default_cfgs = {
    'nat_base': _cfg(
        url='http://ix.cs.uoregon.edu/~alih/nat/checkpoints/CLS/nat_base.pth',
...
def _create_nat_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
...
def nat_base(pretrained=False, **kwargs):
    model_kwargs = dict(
        depths=[3, 4, 18, 5], num_heads=[4, 8, 16, 32], embed_dim=128, mlp_ratio=2, layer_scale=1e-5, kernel_size=7, **kwargs)
    # model = NAT(depths=[3, 4, 18, 5], num_heads=[4, 8, 16, 32], embed_dim=128, mlp_ratio=2,
                ...
                # layer_scale=1e-5, kernel_size=7, **kwargs)
    return _create_nat_transformer("nat_base", pretrained=pretrained, **model_kwargs)